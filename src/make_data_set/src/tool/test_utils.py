...
class Make_mecanum_left():
    ...
    def move(self, x_target, y_target, my_mecanum):
        ...
        dt = 0
        while True:
            if ball_out_check(my_mecanum, "ball_left"):
                self.stop()
                break 
    ...
    def throw_ball(self):
        ...
        self.force = [vx * 0.057 * 1000, 0, vz * 0.057 * 1000 ]

        self.apply_force, self.apply_torque = get_wrench(self.force, self.torque, self.ror_matrix)

        print(self.apply_torque[0] / 1000, self.apply_torque[1] / 1000, self.apply_torque[2] / 1000)
...
def ball_out_check(mecanum, ball_name):
